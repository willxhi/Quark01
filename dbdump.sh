...
echo -e "*init \t $(date)"
DUMPDBDIR=/u01/app/oracle/backups/dumps/
BCKDIR=$_MYPARENTDIR/backup
TWODAYAGOFILE=$(date +%Y%m%d -d "2 day ago")
#51.55
echo -e "*55 \t $(date)"
$_MYDIR/utils/wi_mkdir.sh $BCKDIR/51.55/dump/
rsync -avhPW root@192.168.51.55:$DUMPDBDIR $BCKDIR/51.55/dump/
ssh root@192.168.51.55 << EOSSH
...
rsync -avhPW root@192.168.51.59:$DUMPDBDIR $BCKDIR/51.59/dump/
ssh root@192.168.51.59 << EOSSH
